              ...
              {place.videos && place.videos.length > 0 ? (
                <div className="grid grid-cols-1 gap-3">
                  {place.videos.map((vid, idx) => {
                    ...
                    const ytMatch = vid.match(/(?:youtu\.be\/|youtube\.com\/(?:embed\/|v\/|watch\?v=|watch\?.+&v=|shorts\/))([\w-]{11})/);
                    const ytId = ytMatch ? ytMatch[1] : null;
                    return (
                      <div 
                        key={idx}
                        className="p-3.5 sm:p-4 rounded-xl bg-[#161f30] border border-slate-800 hover:border-[#c29b38]/40 transition-all flex flex-col sm:flex-row sm:items-center justify-between gap-4 group"
                      >
                        <div className="flex items-start sm:items-center gap-3.5 min-w-0">
                          {ytId ? (
                            <a
                              ...
                              target="_blank"
                              rel="noreferrer"
                              className="relative w-24 sm:w-28 aspect-video rounded-lg overflow-hidden shrink-0 bg-slate-900 border border-slate-700/80 group-hover:border-[#c29b38]/60 transition-colors block shadow-md"
                            >
                              <img 
                          ...
                          <div className="min-w-0">
                            <div className="flex items-center gap-2">
                              <p className="text-sm font-semibold text-white group-hover:text-[#f5e3a9] transition-colors">
                                Video tư liệu #{idx + 1}
                              </p>
                            ...
                            {isLink && (
                              <p className="text-xs font-mono text-slate-400 truncate mt-1 max-w-sm sm:max-w-md">
                                {vid}
                              </p>
                            )}
                          </div>
                        ...
                        {isLink && (
                          <a
                            ...
                            target="_blank"
                            rel="noreferrer"
                            className="shrink-0 px-3.5 py-2 rounded-lg bg-red-600 hover:bg-red-500 text-white text-xs font-semibold shadow-md shadow-red-950/40 transition-all flex items-center justify-center gap-1.5 self-stretch sm:self-center"
                          >
                            <ExternalLink className="w-3.5 h-3.5" />
                            Xem trên YouTube
                          </a>
